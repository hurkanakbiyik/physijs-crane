window.Physijs = (function() {
	...
	Physijs.HingeConstraint.prototype.getAngle = function() {
	...
	Physijs.DOFConstraint.prototype.getAngle = function( axis ) {
		if(axis == 0){
			return this.xAngle;	
		}else if(axis == 1){
			return this.yAngle;
	...
	Physijs.Scene.prototype._updateConstraints = function( data ) {
		...
		for ( i = 0; i < ( data.length - 1 ) / CONSTRAINTREPORT_ITEMSIZE; i++ ) {
			...
			if(constraint.hingeAngle != null){
				constraint.hingeAngle = data[ offset + 6 ] ;
			}else if(constraint.xAngle != null && constraint.yAngle != null){
				constraint.xAngle = data[ offset + 6 ] ;
				constraint.yAngle = data[ offset + 7 ] ;
			}
		}
