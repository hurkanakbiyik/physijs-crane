...
var
	...
	CONSTRAINTREPORT_ITEMSIZE = 8, // constraint id, offset object, offset, applied impulse
	constraintreport;
...
public_functions.hinge_getAngle = function( params ) {
	var constraint = _constraints[ params.constraint ];
	console.log(constraint.getHingeAngle());
};
	...
	for ( index in _constraints ) {
		if ( _constraints.hasOwnProperty( index ) ) {
			...
			if(constraint.getConstraintType() == 4){
				constraintreport[ offset + 6 ] = constraint.getHingeAngle();
			}else if(constraint.getConstraintType() == 6){
				constraintreport[ offset + 6 ] = constraint.getAngle(0);
				constraintreport[ offset + 7 ] = constraint.getAngle(1);
			}
		}
	}
